...
graph TB
    subgraph TRAINING_PHASE["TRAINING PHASE (ml_training_pipeline.py)"]
        ...
        T3 --> T4["Train 4 Models\nRandom Forest | SVM | GB | LR"]
        T4 --> T5["Stratified K-Fold CV\n(5-fold)"]
        T5 --> T6["Evaluate\nAccuracy, Precision, Recall, F1, AUC"]
        T6 --> T7["Select Best Model\n(highest F1)"]
...
| **ML Models** | scikit-learn v1.3+ | Implemented | Random Forest, SVM, Gradient Boosting, Logistic Regression |
...
4. **Logistic Regression** (balanced, max_iter=1000) - Interpretable linear baseline
...
### Integration Method
- Audio files are ingested from local filesystem or Google Drive
- Feature extraction is self-contained (no external API calls during inference)
- Model artifacts are serialized with joblib for portability
...
| ML training pipeline (4 models) | Completed | Week 3 |
...
Takes extracted features CSV, trains 4 classifiers, evaluates with cross-validation, saves best model.

| Function | Purpose |
|----------|---------|
| `load_and_preprocess(csv_path)` | Load CSV, handle NaN/Inf, encode labels, fit StandardScaler |
| `analyze_features(csv_path)` | PD vs TBI group statistics, ANOVA F-test feature ranking, bar chart |
| `train_and_evaluate(X, y, ...)` | Train RF/SVM/GB/LR, stratified K-fold CV, confusion matrix, ROC curve |
